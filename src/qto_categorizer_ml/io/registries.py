"""Adapters, savers, loaders, and registers for models in Mlflow Registry."""
...
def uri_for_model(model_name: str, version_or_stage: str) -> str:
    """Create a model URI from a model name and version/stage.

    Args:
        model_name (str): name of the model.
        version_or_stage (str): version or stage.

    Returns:
        str: model URI (models:/model_name/version_or_stage).
    """
    return f"models:/{model_name}/{version_or_stage}"
...
class BuiltinLoader(Loader):
    ...
    class Adapter(Loader.Adapter):
        ...
        def predict(self, inputs: schemas.Inputs) -> schemas.Outputs:
            ...
            outputs = self.model.predict(data=inputs)  # unchecked!
            return schemas.Outputs(outputs, columns=columns)
...
class Register(abc.ABC, pdt.BaseModel, strict=True, frozen=True, extra="forbid"):
    ...
    def register(self, name: str, model_uri: str) -> Version:
        """Register a model given its location (URI).
